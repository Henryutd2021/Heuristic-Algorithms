...
class SimAnneal(object):
    ...
    def random_start(self):
        a, b = self.interval
        return round(a + (b - a) * rn.random_sample())

    def cost_function(self, x):
        def f(x):
            return (((2.5 / 1000) * (x / 1000) ** 1.5) * np.exp(-1 * (x / 1000) ** 2.5)) * x

        v, err = integrate.quad(f, 0, x)

        return (5 * (1-np.exp(-1 * (x / 1000) ** 2.5)) + 1 * np.exp(-1 * (x / 1000) ** 2.5)) / \
                (1 - np.exp(-1 * (x / 1000) ** 2.5)) * v + x * np.exp(-1 * (x / 1000) ** 2.5)
    ...
    def random_neighbour(self, x):
        delta1 = rn.randint(-250, 250)
        return round(self.clip(x + delta1))
...
if __name__ == '__main__':
    sa = SimAnneal(T=1000, M=5, interval=(1, 10000))
